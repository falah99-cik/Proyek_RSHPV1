...
<div class="container-fluid px-4 mt-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <form action="{{ route('admin.temu_dokter.update', data_get($data, 'idreservasi_dokter')) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ data_get($data, 'no_urut') }}" disabled>
                </div>
                    ...
                    <select class="form-control" name="idrole_user">
                        @foreach ($dokter as $d)
                            <option value="{{ data_get($d, 'idrole_user') }}"
                                {{ data_get($d, 'idrole_user') == data_get($data, 'idrole_user') ? 'selected' : '' }}>
                                {{ data_get($d, 'nama') }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="status">
                        <option value="2" {{data_get($data, 'status') == 2 ? 'selected' : ''}}>Menunggu Dokter</option>
                        <option value="1" {{ data_get($data, 'status') == 1 ? 'selected' : '' }}>Diproses Perawat</option>
