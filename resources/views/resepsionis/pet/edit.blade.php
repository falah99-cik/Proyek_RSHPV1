    ...
    <div class="card-body">
        ...
        <form action="{{ route('resepsionis.pet.update', $pet->idpet) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama" class="form-control" value="{{ $pet->nama }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="idpemilik" class="form-control">
                    @foreach ($pemilik as $p)
                        <option 
                            value="{{ data_get($p, 'idpemilik') }}"
                            {{ data_get($p, 'idpemilik') == data_get($pet, 'idpemilik') ? 'selected' : '' }}
                        >
                ...
                <select name="idras_hewan" class="form-control">
                    @foreach ($ras as $r)
                        <option value="{{ $r->idras_hewan }}"
                                {{ $r->idras_hewan == $pet->idras_hewan ? 'selected' : '' }}>
                            {{ $r->nama_ras }}
                        </option>
                    @endforeach
                </select>
